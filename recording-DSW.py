...
class Recording:
    ...
    def analyze(self,analyzer,config,save=True):
        """
        Continuous analysis of recording object with
        ...
        for i_analyzed in range(0, num_window):
            ...
            for i in range(n_bands):
                ...
                # Beginning
                maxes = np.max(self.predictedProbas[i, i_analyzed], axis=1)  # Maximum probability of all classes
                if maxes < config.features['threshold'][0]:
                    # Run analyze again with bigger window.
                    i_start = (i_analyzed * window_length_n) - (i_analyzed * delta) - (window_length_n * .5)
                    i_end = (i_analyzed + 1) * window_length_n - (i_analyzed * delta) + (window_length_n * .5)
                    # Special cases:
                    if i_analyzed == 0:
                        i_start = 0
                        i_end = (i_analyzed + 1) * window_length_n - (i_analyzed * delta) + (window_length_n * 1)
                    elif i_analyzed == (num_window - 1):
                        i_end = np.shape(self.data)[0]
                        i_start = (i_analyzed * window_length_n) - (i_analyzed * delta) - (window_length_n * 1)
                    # Get signal
                    signal = self.data[i_start:i_end]
